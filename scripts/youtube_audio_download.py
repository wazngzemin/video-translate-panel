...
# -*- coding: utf-8 -*-
"""
Download YouTube audio (mp3) with an automatic fallback to browser cookies.
...
def main() -> int:
    ...
    ap.add_argument(
        "--output",
        default="%(title).40s.%(ext)s",
        help="输出文件名模板（会写入 <输出根>/tmp/ 下）",
    )
    ap.add_argument(
        "--force-cookies",
        action="store_true",
    ...
    outtmpl = str(output_root / "tmp" / args.output)
